...
# Use precomputed FAISS vectorstore instead of PDF ingestion
vectorstore_path = "test_faiss_store"
if azure_creds_valid:
    base_retriever = load_vectorstore(vectorstore_path).as_retriever(search_kwargs={"k": k_chunks})
else:
    base_retriever = None

# Load underlying FAISS vectorstore to list files
if azure_creds_valid:
    try:
        ...
        st.error(f"Error loading vectorstore for file listing: {e}")
        files = []
else:
    files = []
...
def generate_answer(state: GraphState) -> GraphState:
    ...
    try:
        ...
        4.  **Contextualized Quoting:** When presenting this key information, include a direct quote (enclosed in double quotation marks) of the most relevant sentence or phrase. **Crucially, also explain the surrounding context from the *same chunk* to clarify the quote's meaning or provide necessary background.** For example, instead of just `"quote..." (filename.pdf, Page X)`, you might say: `The report discusses mitigation strategies, stating, "quote..." (filename.pdf, Page X), which is part of a larger section detailing preventative measures.` Cite the file name and page number in parentheses after the quote, e.g., (ID_OR_Idaho_Power_2022.pdf, Page 31).
