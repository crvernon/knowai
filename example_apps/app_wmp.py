...
def display_logo(column: DeltaGenerator, path: str, alt_text: str) -> None:
    """Display a logo image in a Streamlit column.

    The image located at *path* is rendered inside the given *column* if the
    file exists. When the file cannot be found, a warning message and icon
    are shown instead. Any unexpected errors encountered while loading the
    image are written to the log and surfaced to the user as a generic error
    message.

    Args:
        column: The Streamlit ``DeltaGenerator`` that represents the target
            column where the image—or fallback warning—will be displayed.
        path: Absolute or relative path to the logo image file.
    ...
    try:
        ...
        logging.error(f"Error loading logo {path}: {e}")
        column.error("Logo error.")
...
VECTORSTORE_PATH = os.path.join(os.path.dirname(__file__), "wmp_faiss_text-embedding-3-large")
...
try:
    ...
    logging.error(f"SVG Error: {e}"); st.markdown("<h2 style='text-align:center; margin-top: -50px;'>Wildfire Mitigation Plans Database</h2>", unsafe_allow_html=True)

st.markdown("<div style='text-align:center;'><h4>-- AI Assistant --</h4><em>Your AI Assistant to find answers across multiple reports!</em></div>", unsafe_allow_html=True)
...
if "agent" not in st.session_state:
    ...
    try:
        ...
        if not os.path.exists(VECTORSTORE_PATH):
            ...
            with open(os.path.join(VECTORSTORE_PATH, "index.faiss"), "w") as f: 
                f.write("")
            with open(os.path.join(VECTORSTORE_PATH, "index.pkl"), "w") as f: 
                f.write("")
...
if "available_files" not in st.session_state: # Placeholder for actual file discovery
    st.session_state.available_files = [
        "ID_OR_Idaho_Power_2022.pdf", 
        "OR_City_of_Bandon_2024.pdf", 
        "Anaheim_Public_Utilities_2024.pdf",
        "Anza_Electric_Cooperative_2024.pdf"
    ] 
...
PREDEFINED_QUESTIONS = [
    ...
    "What are common vegetation management practices?",
    "Discuss Public Safety Power Shutoff (PSPS) strategies.", 
    "Summarize asset inspection frequencies."
]
...
with q_col1:
    ...
    custom_question = st.text_input(
        ...
        placeholder="Type here if 'Enter my own question...' is selected"
    )
with q_col2:
    st.markdown("<br/>", unsafe_allow_html=True)
    start_chat_button = st.button(
        "Start Chat", 
        type="primary", 
...
if follow_up_prompt := st.chat_input(
    "Ask a follow-up question...", 
    key="follow_up_chat_input", 
